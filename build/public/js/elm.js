...
Elm.Main.make = function (_elm) {
   ...
   var _op = {};
   var drawSq = function (m) {
      return A2($Graphics$Collage.rotate,
      m.rotation,
      A2($Graphics$Collage.move,m.position,A2($Graphics$Collage.filled,$Color.black,$Graphics$Collage.square(m.size))));
   };
   var squareWith = F3(function (pos,size,rotation) {    return {position: pos,size: size,rotation: rotation};});
   var Sq = F3(function (a,b,c) {    return {position: a,size: b,rotation: c};});
   var arrangeInCircle = F4(function (w,h,r,i) {
      var ring = $Basics.toFloat(i / 40 | 0);
      var ringPct = ring / $Basics.toFloat(480 / 40 | 0);
      var size = (1 + ringPct) * 8;
      var pctInRing = $Basics.toFloat(A2($Basics._op["%"],i,40)) / 40;
      var tau = 2 * $Basics.pi;
      var position = {ctor: "_Tuple2",_0: $Basics.cos(pctInRing * tau) * (r * ringPct),_1: $Basics.sin(pctInRing * tau) * (r * ringPct)};
      var rotation = (1 - pctInRing) * tau;
      return A3(squareWith,position,size,rotation);
   });
   var main = function () {
      var h = 500;
      var w = 500;
      ...
      w,
      h,
      $List.concat(_U.list([_U.list([A2($Graphics$Collage.filled,$Color.grey,$Graphics$Collage.square(500))])
                           ,$Array.toList(A2($Array.map,drawSq,A2($Array.initialize,1480,A3(arrangeInCircle,w,h,w / 2 * 0.9))))])));
   }();
   return _elm.Main.values = {_op: _op,main: main,arrangeInCircle: arrangeInCircle,Sq: Sq,squareWith: squareWith,drawSq: drawSq};
};
